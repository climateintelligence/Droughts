...
class HierarchicalClustering:
    ...
    def compute_clusters(self):
        ...
        progress_bar = tqdm(total=self.df.shape[1], position=0, leave=True, smoothing=0)
        ...
        while self.enough_strength(self.neighbours_strength_dict[max_strength]):
            ...
            if elem1.startswith("cluster") and not elem2.startswith("cluster"):
                ...
                self.last_clusters.append([elem1, elem2])
        ...
        for key in self.neighbours_strength_dict:
            ...
            if not elem1.startswith("cluster") and elem1 not in self.last_clusters:
                self.last_clusters.append(elem1)

            if not elem2.startswith("cluster") and elem2 not in self.last_clusters:
    ...
    def get_neighbours_strength(self, cluster_index, cluster_mean, elem2):
        strength = ''

        if self.method == 'correlation':
            value2 = self.df[self.last_clusters[int(elem2.split("_")[1])]].mean(axis=1) if elem2.startswith("cluster") else self.df[elem2]
            ...
            if elem2.startswith("cluster"):
                cluster2 = self.df[self.last_clusters[int(elem2.split("_")[1])]]
                for elem_cluster1 in cluster1.columns:
                    for elem_cluster2 in cluster2.columns:
                        strength_temp = np.corrcoef(self.df[elem_cluster1], self.df[elem_cluster2])[0, 1]
                        if strength_temp < strength:
                            strength = strength_temp

            else:
                for elem_cluster1 in cluster1.columns:
                    strength_temp = np.corrcoef(self.df[elem_cluster1], self.df[elem2])[0, 1]
                    if strength_temp < strength:
                        strength = strength_temp    
    ...
    def refresh_corr_values(self, elem1_cluster, elem2_cluster, cluster_index):
        ...
        for key in list(self.neighbours_strength_dict.keys()):
            ...
            if elem1 in (elem1_cluster, elem2_cluster):
                ...
                strength = self.get_neighbours_strength(cluster_index, cluster_mean, elem2)
                ...
                strength = self.get_neighbours_strength(cluster_index, cluster_mean, elem1)
        ...
        for key in list(self.neighbours_strength_dict.keys()):
            ...
            for cluster_n in range(to_be_removed+1, len(self.last_clusters)): 
                ...
                if f'cluster_{cluster_n}' in key:
                    ...
                    new_key = tuple([f'cluster_{cluster_n-1}' if elem == f'cluster_{cluster_n}' else elem for elem in decomposed_key])
