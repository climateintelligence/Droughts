...
def prepare_target_binary(colname,max_train='2013-11-22', max_val='2018-04-10', max_test='2022-06-24', path='/Users/paolo/Documents/OneDrive - Politecnico di Milano/droughts/csv_VHI/Emiliani2.csv'):
    target_df = pd.read_csv(path)
    target_df = target_df.rename({'Unnamed: 0':'date'},axis=1)
    target_df_train = target_df.loc[target_df['date']<=max_train,:].copy()
    target_df_val = target_df.loc[(target_df['date']>max_train) & (target_df['date']<=max_val),:].copy()
    ...
    target_df_train['mean_std'] = target_df_train.apply(lambda x: 1 if x['mean']>0.5 else 0, axis=1)
    target_df_val['mean_std'] = target_df_val.apply(lambda x: 1 if x['mean']>0.5 else 0, axis=1)
    target_df_test['mean_std'] = target_df_test.apply(lambda x: 1 if x['mean']>0.5 else 0, axis=1)
    target_df_trainVal['mean_std'] = target_df_trainVal.apply(lambda x: 1 if x['mean']>0.5 else 0, axis=1)
...
def tuning_GenLinCFA(df_trainVal_unfolded_std_withTar,neigh, eps):
    n_iter = 0
    output = GenLinCFA(df_trainVal_unfolded_std_withTar,'mean_std', eps, -5 , neigh, eps).compute_clusters()
    while (((len(output)<3) | (len(output)>10)) & (n_iter<100)):
        output = GenLinCFA(df_trainVal_unfolded_std_withTar,'mean_std', eps, -5 , neigh, eps).compute_clusters()
        if len(output)<3: eps -= 0.005#0.01
        if len(output)>10: eps += 0.005#0.01
        n_iter +=1
        print(len(output))
    return output
...
def aggregate_unfolded_data(path,colnames, target_df_trainVal, eps, multiple=False, max_train='2013-11-22', max_val='2018-04-10', max_test='2022-12-31', neigh=1):
    ...
    for col in colnames:
        ...
        output = tuning_GenLinCFA(df_trainVal_unfolded_std_withTar,neigh, eps)
...
def FS_with_logisticWrapper(aggregate_trainVal, target_df_train, target_df_val, max_feat, val_len=200):
    aggregate_train = aggregate_trainVal.iloc[:-val_len]
    aggregate_val = aggregate_trainVal.iloc[-val_len:]  
